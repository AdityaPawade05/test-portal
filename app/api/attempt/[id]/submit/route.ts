...
export async function POST(
  ...
  try {
    ...
    if (isSqsEnabled()) {
      await enqueueJob({ type: "SCORE_ATTEMPT", attemptId: id });
    }

    // Re-runnable/idempotent — safe to compute or rehydrate (spec section 6).
    ...
    console.error("[POST /api/attempt/[id]/submit] Error:", err);
